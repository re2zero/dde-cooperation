...
REM BonjourPSSetup.exe is a universal installer that supports both 32-bit and 64-bit systems
set BONJOUR_URL=http://binaries.symless.com/bonjour/BonjourPSSetup.exe
...
if not exist "%DEPS_DIR%\BonjourPSSetup.exe" (
    powershell -Command "& {Invoke-WebRequest -Uri '%BONJOUR_URL%' -OutFile '%DEPS_DIR%\BonjourPSSetup.exe'}"
    if errorlevel 1 (
        echo Failed to download BonjourPSSetup.exe
        echo You can download it manually from: %BONJOUR_URL%
    ) else (
        echo Downloaded: %DEPS_DIR%\BonjourPSSetup.exe
    )
) else (
    echo Already exists: %DEPS_DIR%\BonjourPSSetup.exe
)

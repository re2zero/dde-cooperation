...
if exist "%VCPKG_ROOT%\vcpkg.exe" (
    ...
    if ERRORLEVEL 1 (
        echo Warning: vcpkg dependency installation failed, but CMake may still succeed
    )
) else (
    echo vcpkg.exe not found at %VCPKG_ROOT%, relying on CMake to install dependencies
)
...
if exist output\%B_BUILD_TYPE% (
    ...
    copy "%B_BONJOUR%\%BONJOUR_FILE%" installer-inno\%COO_PROJECT%\ > NUL
    REM Copy VC Redist from dependencies directory if it exists there
    if exist "%B_BONJOUR%\%VCREDIST_FILE%" (
        copy "%B_BONJOUR%\%VCREDIST_FILE%" installer-inno\%COO_PROJECT%\ > NUL
    )
    REM If VC Redist was copied to output directory, move it
    if exist output\%COO_PROJECT%\%B_BUILD_TYPE%\%VCREDIST_FILE% (
        move output\%COO_PROJECT%\%B_BUILD_TYPE%\%VCREDIST_FILE% installer-inno\%COO_PROJECT%\ > NUL
    )
    ...
    copy "%B_BONJOUR%\%BONJOUR_FILE%" installer-inno\%DT_PROJECT%\ > NUL
    REM Copy VC Redist from dependencies directory if it exists there
    if exist "%B_BONJOUR%\%VCREDIST_FILE%" (
        copy "%B_BONJOUR%\%VCREDIST_FILE%" installer-inno\%DT_PROJECT%\ > NUL
    )
    REM If VC Redist was copied to output directory, move it
    if exist output\%DT_PROJECT%\%B_BUILD_TYPE%\%VCREDIST_FILE% (
        move output\%DT_PROJECT%\%B_BUILD_TYPE%\%VCREDIST_FILE% installer-inno\%DT_PROJECT%\ > NUL
    )
) else (

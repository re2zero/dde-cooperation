...
void HandleIpcService::doApplyTransfer(const QString &appname, const QString& targetname, const QString& machinename, const int type)
{
...
bool HandleIpcService::doStartShare(const QString& appname, const QString &msg)
{
    ShareServerConfig config;
    co::Json config_json;
    if (!config_json.parse_from(msg.toStdString())) {
        ELOG << "doStartShare parse error: " << msg.toStdString();
        return false;
    }
    ...
    // 读取相应的配置配置Barrier
    if (!ShareCooperationServiceManager::instance()->server()->setServerConfig(config)) {
        // ShareEvents ev;
    ...
    // 启动服务器
    QString jsonData = startEvent.as_json().str().c_str();
    return ShareCooperationServiceManager::instance()->startServer(jsonData);
}
...
void HandleIpcService::doStopShare(const QString& appname, const QString &targetname, const int flags)
{
    ...
    // 停止自己的共享
    if (flags == ShareStopFlag::SHARE_STOP_ALL) {
        ShareCooperationServiceManager::instance()->stop();
        DiscoveryJob::instance()->updateAnnouncShare(true);
    } else if (flags == ShareStopFlag::SHARE_STOP_CLIENT) {
        stopEvent.flags = ShareStopFlag::SHARE_STOP_SERVER;
        ShareCooperationServiceManager::instance()->client()->stopBarrier();
    } else {
        stopEvent.flags = ShareStopFlag::SHARE_STOP_CLIENT;

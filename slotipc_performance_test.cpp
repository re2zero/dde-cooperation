...
public:
    ...
    void runLocalSocketTest() {
        ...
        // Test latency
        results.localLatency = testLatency(&interface, "Local Socket");
        
        // Test throughput
        results.localThroughput = testThroughput(&interface, "Local Socket");
    ...
    void runTcpSocketTest() {
        ...
        // Test latency
        results.tcpLatency = testLatency(&interface, "TCP Socket");
        
        // Test throughput
        results.tcpThroughput = testThroughput(&interface, "TCP Socket");
        
        service.close();
    }

private:
    double testLatency(SlotIPCInterface* interface, const QString& testType) {
    ...
    double testThroughput(SlotIPCInterface* interface, const QString& testType) {
        ...
        timer.start();
        for (int i = 0; i < iterations; ++i) {
            // Use synchronous call with return value for more reliable measurement
            int result;
            interface->call("add", Q_RETURN_ARG(int, result), Q_ARG(int, 1), Q_ARG(int, 2));
        }
...
public:
    void testLocalFunctionCall() {
        ...
        averageLatency = static_cast<double>(elapsed) / iterations;
        qDebug() << QString("Local function call average latency: %1 ms").arg(averageLatency, 0, 'f', 6);
    }
...
int main(int argc, char *argv[])
{
    ...
    qDebug() << "";
    qDebug() << "=== Test Summary ===";
    qDebug() << "All tests completed. Results comparison:";
    qDebug() << QString("1. Local function call: %1 ms (baseline)").arg(benchmark.getAverageLatency(), 0, 'f', 6);
    qDebug() << QString("2. SlotIPC Local Socket: %1 ms (%2x slower than local)").arg(perfTest.results.localLatency, 0, 'f', 3).arg(perfTest.results.localLatency / benchmark.getAverageLatency(), 0, 'f', 0);
    qDebug() << QString("3. SlotIPC TCP Socket: %1 ms (%2x slower than local)").arg(perfTest.results.tcpLatency, 0, 'f', 3).arg(perfTest.results.tcpLatency / benchmark.getAverageLatency(), 0, 'f', 0);
    qDebug() << "";
    qDebug() << "Throughput comparison:";
    qDebug() << QString("- Local Socket: %1 ops/sec").arg(perfTest.results.localThroughput, 0, 'f', 0);
    qDebug() << QString("- TCP Socket: %1 ops/sec").arg(perfTest.results.tcpThroughput, 0, 'f', 0);
    qDebug() << "";
